...
While we reviewed, discussed and improved the code together, we had a clear separation of responsibility. Petr Sabovčik worked on the files \textit{main.c} and \textit{manager.c}. He created the shell with command line input processing and process execution. Pascal Wohlwender on the other hand worked on \textit{command.c}, the header files and the proshdom executable. He created the productivity mode with process and domain blocking.
